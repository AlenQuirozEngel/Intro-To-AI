...
# Train and predict using machine learning (Random Forest)
...
# Compare the rule-based and machine learning systems
def compare_systems(processed_data, rule_based_df, heart_attack_probs, accident_probs, death_probs, death_preds):
    print("\n--- Comparison of Rule-Based and Machine Learning Systems ---")
    
    # Rule-based accuracy
    heart_attack_rule_acc = accuracy_score(processed_data['heart_attack_count'] > 0, rule_based_df['pred_heart_attack'])
    accident_rule_acc = accuracy_score(processed_data['accident_count'] > 0, rule_based_df['pred_accident'])
    death_rule_acc = accuracy_score(processed_data['died'], rule_based_df['pred_death'])
    
    print(f"Rule-Based Heart Attack Accuracy: {heart_attack_rule_acc:.2%}")
    print(f"Rule-Based Accident Accuracy: {accident_rule_acc:.2%}")
    print(f"Rule-Based Death Accuracy: {death_rule_acc:.2%}")
    
    # Confusion matrices for rule-based system
    print("Rule-Based Confusion Matrix for Heart Attack:")
    print(confusion_matrix(processed_data['heart_attack_count'] > 0, rule_based_df['pred_heart_attack']))
    
    print("Rule-Based Confusion Matrix for Accidents:")
    print(confusion_matrix(processed_data['accident_count'] > 0, rule_based_df['pred_accident']))
    
    print("Rule-Based Confusion Matrix for Death:")
    print(confusion_matrix(processed_data['died'], rule_based_df['pred_death']))
    ...
    # Confusion matrices for machine learning system
    print("ML Confusion Matrix for Heart Attack:")
    print(confusion_matrix(processed_data['heart_attack_count'] > 0, heart_attack_probs > 50))
    
    print("ML Confusion Matrix for Accidents:")
    print(confusion_matrix(processed_data['accident_count'] > 0, accident_probs > 50))
    
    print("ML Confusion Matrix for Death:")
...
def main():
    ...
    if data is not None:
        ...
        compare_systems(processed_data, rule_based_df, heart_attack_probs, accident_probs, death_probs, death_preds)
