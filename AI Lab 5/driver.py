...
def plot_convolution_2D(x: np.ndarray) -> None:
    ...
    # Setup kernels
    kernels = [
        # Sharpening kernel (center emphasizes more than surroundings)
        np.array(
            [
                [0, -1, -1, -1, 0],
                [-1, 2, 2, 2, -1],
        ...
        # Gaussian blur kernel (5x5 normalized)
        np.array(
            [
                [1, 4, 7, 4, 1],
                [4, 16, 26, 16, 4],
        ...
        # Laplacian kernel (second derivative operator for edge detection)
        np.array(
            [
                [0, 0, -1, 0, 0],
                [0, -1, -2, -1, 0],
        ...
        # Embossing kernel (emphasizes edges with directional light effect)
        np.array(
            [
                [-2, -1, 0, 1, 2],
                [-1, 0, 1, 2, 1],
    ...
    conv = ls.Convolution2D(
        ...
        stride=1,
        padding=4,  # Adjust padding for kernel size 5
    ...
    conv.load_weights(
        {
            "kernel:0": kernel,  # Shape == [kernel_size, kernel_size, input_channel_count, output_channel_count]
            "bias:0": bias,
        }
    )  # Shape == [output_channel_count]
    ...
    def draw_plots():
        ...
        gs = figure.add_gridspec(
            output_channel_count, 3, width_ratios=[1, 1 / output_channel_count, 1]
        )
        ...
        for f in range(output_channel_count):
            ...
            axes.clear()
            axes.imshow(
                conv.weights["kernel:0"][:, :, :, f] / np.max(conv.weights["kernel:0"])
            )
            axes.set_ylabel(f)
            axes.set_xticks([])
            axes.set_yticks([])
    ...
    image_button = Button(
        image_button_position, "Next Image", color="white", hovercolor="gray"
    )
    image_button.on_clicked(next_image_click)

    kernel_button_position = plt.axes([0.455, 0.01, 0.12, 0.07])  # x,y, width, height
    kernel_button = Button(
        kernel_button_position, "Next Kernel", color="white", hovercolor="gray"
    )
...
if __name__ == "__main__":
    ...
    with open("x_test.npy", "rb") as f:
        x = np.load(f)[
            :25, :
        ]  # Only take the first few instances for performance reasons
    ...
    if False:
        ...
        layers = [
            ...
            ls.Dense(
                name="dense_1",
                input_dimensionality=height * width * channel_count,
    ...
    if False:
        ...
        layers = [
            ...
            ls.MaxPooling2D(
                name="pool2d", pooling_size=2, stride=2, padding=0
            ),  # Changed stride to 2
            ls.Convolution2D(
                name="conv2d_1",
            ...
            ls.MaxPooling2D(
                name="pool2d_1", pooling_size=2, stride=2, padding=0
            ),  # Changed stride to 2
            ls.Flatten(name="flat"),
            ls.Dense(
                name="dense",
                input_dimensionality=2304,  # Updated input_dimensionality to 2304
                output_dimensionality=10,  # Updated output_dimensionality to 10
                activation_function="softmax",
            ),
        ]
    ...
    if False:
        ...
        # Add the GRU layer (input: 2D coordinates, output: 32-dimensional hidden state)
        gru_layer = ls.GatedRecurrentUnit(
            name="gru",
        ...
        # Add the Dense layer (input: 32-dimensional hidden state, output: num_classes)
        num_classes = 10
        dense_layer = ls.Dense(
            name="dense",
            input_dimensionality=32,
            output_dimensionality=num_classes,
            activation_function="softmax",
        )
