...
from tabulate import (
    tabulate,
)  # For printing model summaries. This is useful for debugging but not strictly necessary.
...
class Dense(Layer):
    ...
    def __init__(
        ...
        # Initialize weights
        self.weights["kernel:0"] = np.random.uniform(
            low=-1.0, high=1.0, size=[input_dimensionality, output_dimensionality]
        )
        self.weights["bias:0"] = np.random.uniform(
            low=-1.0, high=1.0, size=[output_dimensionality]
        )
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        ...
        return self.activation_function(
            np.dot(x, self.weights["kernel:0"]) + self.weights["bias:0"]
        )
...
class Convolution2D(Layer):
    ...
    def __init__(
        ...
        self.weights["kernel:0"] = np.random.uniform(
            low=-1.0,
            high=1.0,
            size=[kernel_size, kernel_size, input_channel_count, output_channel_count],
        )
        self.weights["bias:0"] = np.random.uniform(
            low=-1.0, high=1.0, size=[output_channel_count]
        )
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        ...
        # Prepare weights
        kernel = np.repeat(
            self.weights["kernel:0"][np.newaxis, :, :, :, :],
            repeats=instance_count,
            axis=0,
        )
        bias = np.repeat(
            self.weights["bias:0"][np.newaxis, :], repeats=instance_count, axis=0
        )

        # Iterate images to convolve
        y_hat = self.activation_function(
            Convolution2D.__iterate__(
                x=x,
                patch_size=self.kernel_size,
                stride=self.stride,
                padding=self.padding,
                output_channel_count=self.output_channel_count,
                function=lambda x_ij: np.sum(
                    np.sum(
                        np.sum(
                            np.repeat(
                                x_ij[:, :, :, :, np.newaxis],
                                repeats=self.output_channel_count,
                                axis=-1,
                            )
                            * kernel,
                            axis=1,
    ...
    def __iterate__(
        ...
        # Calculate new dimensions after padding and striding
        new_height = (x_padded.shape[1] - patch_size) // stride + 1
        new_width = (x_padded.shape[2] - patch_size) // stride + 1

        # Initialize output
        output = np.zeros((instance_count, new_height, new_width, output_channel_count))

        # Iterate the height and width of the images
        for i in range(new_height):
            for j in range(new_width):
                ...
                output[:, i, j, :] = function(patch)

        # Output
        return output

        # Remove the pass once you put your own code above
...
class MaxPooling2D(Layer):
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        ...
        y_hat = Convolution2D.__iterate__(
            ...
            function=lambda patch: np.max(patch, axis=(1, 2)),
        )
...
class GatedRecurrentUnit(Layer):
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        ...
        x = np.concatenate(
            [np.zeros([instance_count, 1, input_dimensionality], dtype=x.dtype), x],
            axis=1,
        )

        # Extract weights
        Wxz = self.weights["kernel:0"][
            :, : self.output_dimensionality
        ]  # Shape == (input_dimensionality, output_dimensionality)
        Wxr = self.weights["kernel:0"][
            :, self.output_dimensionality : 2 * self.output_dimensionality
        ]  # Shape == (input_dimensionality, output_dimensionality)
        Wxh = self.weights["kernel:0"][
            :, 2 * self.output_dimensionality :
        ]  # Shape == (input_dimensionality, output_dimensionality)

        Whz = self.weights["kernel:1"][
            :, : self.output_dimensionality
        ]  # Shape == (output_dimensionality, output_dimensionality)
        Whr = self.weights["kernel:1"][
            :, self.output_dimensionality : 2 * self.output_dimensionality
        ]  # Shape == (output_dimensionality, output_dimensionality)
        Whh = self.weights["kernel:1"][
            :, 2 * self.output_dimensionality :
        ]  # Shape == (output_dimensionality, output_dimensionality)

        # Iterate time steps, update h vector
        h = np.zeros(
            (instance_count, 1 + time_step_count, self.output_dimensionality),
            dtype=np.float32,
        )
        for t in range(1, time_step_count + 1):
            ...
            ht_minus_1 = h[
                :, t - 1, :
            ]  # Shape == [instance_count, output_dimensionality].

            ########## YOU CODE STARTS HERE
            # Update Gate (zₜ)
            zt = activation_functions["sigmoid"](
                np.dot(xt, Wxz) + np.dot(ht_minus_1, Whz)
            )

            # Reset Gate (rₜ)
            rt = activation_functions["sigmoid"](
                np.dot(xt, Wxr) + np.dot(ht_minus_1, Whr)
            )

            # Candidate Hidden State (ĥₜ)
            h_tilde_t = activation_functions["tanh"](
                np.dot(xt, Wxh) + np.dot(rt * ht_minus_1, Whh)
            )

            # Final Hidden State (hₜ)
            h[:, t, :] = zt * ht_minus_1 + (1 - zt) * h_tilde_t
            ########## YOUR CODE ENDS HERE
...
class Model:
    ...
    def print_summary(self, x):
        ...
        table = [
            ["Layer Index", "Layer Class", "Layer Name", "Output shape", "Weight Count"]
        ]
